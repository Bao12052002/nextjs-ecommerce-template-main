...
const Header = ({ menuData, logoUrl }: { menuData: WPMenuItem[], logoUrl?: string | null }) => {
  const [searchQuery, setSearchQuery] = useState("");
  const [navigationOpen, setNavigationOpen] = useState(false);
  const [stickyMenu, setStickyMenu] = useState(false);
  const siteLogo = logoUrl || "/images/logo/logo.png";
  ...
  // Helper: Xử lý menu từ WordPress (Nếu API lỗi thì dùng mảng rỗng)
          ...
          <div className="xl:w-auto flex-col sm:flex-row w-full flex sm:justify-between sm:items-center gap-5 sm:gap-10">
           <Link className="flex-shrink-0" href="/">
              ...
              <Image
                ...
                width={90}
                height={10}
                className="w-auto h-auto object-contain" // Thêm object-contain để ảnh không bị méo
                priority // Logo nên được ưu tiên load
              />
            </Link>
          ...
          <div className="flex items-center justify-between">
            ...
            <div
              className={`w-[288px] absolute right-4 top-full xl:static xl:w-auto h-0 xl:h-auto invisible xl:visible xl:flex items-center justify-between ${
                navigationOpen &&
                `!visible bg-white shadow-lg border border-gray-3 !h-auto max-h-[400px] overflow-y-scroll rounded-md p-5 z-50`
              }`}
            >
              {/* */}
              <nav>
                <ul className="flex xl:items-center flex-col xl:flex-row gap-5 xl:gap-6">
                  {/* Sử dụng menuList từ WordPress */}
                  {menuList.map((menuItem, i) =>
                    // Kiểm tra menu con dựa trên cấu trúc WPGraphQL (childItems.nodes)
