...
import { Product } from "@/types/product"; // Hoặc ProductNode tùy type bạn dùng chung
...
// Nếu bạn dùng type ProductNode từ GraphQL thì đổi type ở đây
const SingleItem = ({ product }: { product: Product }) => {
  ...
  const handleAddToCart = () => {
    dispatch(
      addItemToCart({
        ...product,
        title: productName,
        price: typeof displayPrice === 'string' ? parseFloat(displayPrice.replace(/[^0-9.]/g, '')) : displayPrice,
        quantity: 1,
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] min-h-[403px]">
        ...
        {/* Phần phải: Action Buttons (Slide từ phải sang) */}
