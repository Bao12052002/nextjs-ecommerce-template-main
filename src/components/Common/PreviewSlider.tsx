...
import { useCallback, useEffect, useRef } from "react"; // 👈 Import thêm useEffect
...
const PreviewSlider = () => {
  ...
  // Lấy dữ liệu từ Redux (bao gồm cả initialIndex ta vừa gửi từ QuickView)
  const data: any = useAppSelector((state) => state.productDetailsReducer.value);

  const sliderRef = useRef<any>(null);

  // --- XỬ LÝ DỮ LIỆU ẢNH ---
  const mainImg = data?.image?.sourceUrl || data?.img;
  const galleryNodes = data?.galleryImages?.nodes?.map((n: any) => n.sourceUrl) || [];

  let images = [...galleryNodes];
  ...
  // --- LOGIC MỚI: Tự động nhảy tới ảnh đang xem ---
  useEffect(() => {
    // Chỉ chạy khi Modal mở ra và Swiper đã sẵn sàng
    if (isModalPreviewOpen && sliderRef.current && sliderRef.current.swiper) {
      const indexToGo = data?.initialIndex || 0; // Lấy index từ QuickView gửi sang
      
      // swiper.slideTo(index, speed, runCallbacks)
      // speed = 0 để nhảy ngay lập tức, không cần hiệu ứng trượt
      sliderRef.current.swiper.slideTo(indexToGo, 0); 
    }
  }, [isModalPreviewOpen, data]); // Chạy lại khi modal mở hoặc data thay đổi
  ...
  return (
    <div className={`preview-slider fixed w-full h-screen z-999999 inset-0 flex justify-center items-center bg-[#000000F2] bg-opacity-70`}>
      {/* Nút đóng */}
      <button
        onClick={() => closePreviewModal()}
        aria-label="button for close modal"
        className="absolute top-0 right-0 sm:top-6 sm:right-6 flex items-center justify-center w-10 h-10 rounded-full ease-in duration-150 text-white hover:text-meta-5 z-20"
      >
      ...
      {/* Điều hướng */}
      <div>
        <button className="rotate-180 absolute left-4 sm:left-10 p-5 cursor-pointer z-10 text-white hover:text-meta-5 transition-colors" onClick={handlePrev}>
        ...
        <button className="absolute right-4 sm:right-10 p-5 cursor-pointer z-10 text-white hover:text-meta-5 transition-colors" onClick={handleNext}>
          <svg width="36" height="36" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fillRule="evenodd" clipRule="evenodd" d="M14.5918 5.92548C14.9091 5.60817 15.4236 5.60817 15.7409 5.92548L22.2409 12.4255C22.5582 12.7428 22.5582 13.2572 22.2409 13.5745L15.7409 20.0745C15.4236 20.3918 14.9091 20.3918 14.5918 20.0745C14.2745 19.7572 14.2745 19.2428 14.5918 18.9255L19.7048 13.8125H4.33301C3.88428 13.8125 3.52051 13.4487 3.52051 13C3.52051 12.5513 3.88428 12.1875 4.33301 12.1875H19.7048L14.5918 7.07452C14.2745 6.75722 14.2745 6.24278 14.5918 5.92548Z" fill="currentColor" />
          </svg>
        </button>
      ...
      <Swiper 
        ...
        // Thêm prop này để Swiper nhận biết thay đổi
        observer={true} 
        observeParents={true}
      >
