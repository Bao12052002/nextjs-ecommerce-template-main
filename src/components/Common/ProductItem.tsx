...
const ProductItem = ({ product }: { product: Product }) => {
  ...
  // 👇 1. XỬ LÝ DỮ LIỆU AN TOÀN (Lấy từ code mới)
  if (!product) return null;

  const productName = product.name || product.title || "Sản phẩm";
  // Ưu tiên lấy ảnh từ WordPress (sourceUrl), nếu không có thì lấy từ mảng previews, cuối cùng là placeholder
  const productImg = product.image?.sourceUrl || product.imgs?.previews?.[0] || "/images/placeholder.png";
  
  // Xử lý giá tiền (loại bỏ ký tự lạ nếu cần)
  ...
  // 👇 3. GIAO DIỆN HTML/CSS/SVG (Giữ nguyên gốc 100%)
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-[#F6F7FB] min-h-[270px] mb-4">
        {/* Render ảnh với dữ liệu mới */}
        <Link href={linkHref}>
            <Image 
                src={productImg} 
                alt={productName} 
                width={250} 
                height={250}
                className="object-contain" // Thêm chút style để ảnh không bị méo
            />
        </Link>
      ...
      <div className="flex items-center gap-2.5 mb-2">
        <div className="flex items-center gap-1">
          {/* Loop render 5 ngôi sao (Giữ nguyên icon static như cũ) */}
          {[...Array(5)].map((_, i) => (
