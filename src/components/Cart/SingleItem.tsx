...
import { formatVND } from "@/utils/formatCurrency"; // 👇 Import hàm format tiền tệ

const SingleItem = ({ item }: { item: any }) => {
  const dispatch = useDispatch<AppDispatch>();

  // 👇 Sử dụng trực tiếp quantity từ Redux item, không cần useState cục bộ
  // Điều này đảm bảo tính nhất quán dữ liệu ("Single Source of Truth")
  ...
  const handleIncreaseQuantity = () => {
    dispatch(updateCartItemQuantity({ id: item.id, quantity: currentQuantity + 1 }));
  };
  ...
  // Tính giá hiển thị (ưu tiên giá giảm)
  const priceToDisplay = item.discountedPrice || item.price || 0;
  const lineTotal = priceToDisplay * currentQuantity;
      ...
      <div className="min-w-[400px]">
        <div className="flex items-center justify-between gap-5">
          <div className="w-full flex items-center gap-5.5">
            ...
            <div>
              <h3 className="text-dark ease-out duration-200 hover:text-blue font-medium">
      ...
      <div className="min-w-[275px]">
        <div className="w-max flex items-center rounded-md border border-gray-3">
          ...
          <button
            ...
            className="flex items-center justify-center w-11.5 h-11.5 ease-out duration-200 hover:text-blue text-gray-500"
          >
          ...
          {/* Số lượng */}
          <span className="flex items-center justify-center w-16 h-11.5 border-x border-gray-4 text-dark font-medium">
            {currentQuantity}
          </span>
