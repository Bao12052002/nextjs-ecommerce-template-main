...
const SingleGridItem = ({ product }: { product: ProductNode }) => {
  ...
  // Format giá (bỏ &nbsp;)
  const formatPrice = (p: string | null | undefined) => (p || "").replace(/&nbsp;/g, ' ');
  
  // Logic hiển thị giá: Nếu đang sale thì hiện giá Sale, ngược lại hiện giá thường
  const displayPrice = formatPrice(product.salePrice || product.price || "Liên hệ");
    ...
    <div className="group">
      ...
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        <Link href={productUrl}>
            <Image 
                ...
                width={250} 
                height={250}
                className="object-contain" // Thêm class này để ảnh không bị méo
            />
        </Link>
      ...
      <div className="flex items-center gap-2.5 mb-2">
        <div className="flex items-center gap-1">
          {[...Array(5)].map((_, i) => (
            // Dùng Inline SVG thay vì Image để đảm bảo hiển thị luôn mà không cần file ảnh
            <svg key={i} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z" fill="#FBB040"/>
            </svg>
          ))}
        </div>
