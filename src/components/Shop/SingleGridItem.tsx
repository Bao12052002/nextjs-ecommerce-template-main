...
import { formatVND } from "@/utils/formatCurrency"; // 👇 Import hàm format mới

const SingleGridItem = ({ product }: { product: ProductNode }) => {
  const { openModal } = useModalContext();
  const dispatch = useDispatch<AppDispatch>();
  const { handleAddToCart, loading } = useAddToCart(product);
  ...
  // 👇 SỬ DỤNG HÀM FORMAT VND
  // Ưu tiên salePrice nếu có, ngược lại dùng price
  const displayPrice = formatVND(product.salePrice || product.price);
  const regularPrice = formatVND(product.regularPrice);

  // Xử lý Quick View (Map lại dữ liệu chuẩn)
  const handleQuickViewUpdate = () => {
    dispatch(updateQuickView({
      ...product,
      id: product.databaseId || product.id,
      // 👇 Thay regex cũ bằng /\D/g để lấy đúng số nguyên
      price: product.price ? parseFloat(product.price.replace(/\D/g, "")) : 0,
      discountedPrice: product.salePrice ? parseFloat(product.salePrice.replace(/\D/g, "")) : 0,
    }));
    openModal();
  };
  ...
  const handleItemToWishList = () => {
    dispatch(addItemToWishlist({ 
        id: product.databaseId || Number(product.id),
        title: product.name,
        // 👇 Thay regex cũ bằng /\D/g
        price: product.price ? parseFloat(product.price.replace(/\D/g, "")) : 0,
        discountedPrice: product.salePrice ? parseFloat(product.salePrice.replace(/\D/g, "")) : 0,
        imgs: { thumbnails: [imgUrl], previews: [imgUrl] },
        quantity: 1,
        status: "available" 
    }));
  };
        ...
        <div className="absolute left-0 bottom-0 translate-y-full w-full flex items-center justify-center gap-2.5 pb-5 ease-linear duration-200 group-hover:translate-y-0">
          ...
          <button 
            ...
            className={`inline-flex font-medium text-custom-sm py-[7px] px-5 rounded-[5px] text-white ease-out duration-200 
                ${loading ? "bg-blue-light cursor-not-allowed" : "bg-blue hover:bg-blue-dark"}`}
          >
            {loading ? (
        ...
        <div className="flex items-center gap-1">
          {[...Array(5)].map((_, i) => (
            <svg key={i} width="15" height="15" viewBox="0 0 15 15" fill="none">
              <path d="M7.5 0L9.18386 5.18237H14.6329L10.2245 8.38525L11.9084 13.5676L7.5 10.3647L3.09161 13.5676L4.77547 8.38525L0.367076 5.18237H5.81614L7.5 0Z" fill={i < (product.averageRating || 0) ? "#FBB040" : "#E0E0E0"}/> 
            </svg>
          ))}
        </div>
